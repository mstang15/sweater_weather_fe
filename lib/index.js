...
function logIn(){
  toggle(document.getElementById('login-button'));
...
function currentWeather(response){
  ...
  weatherCurrentTemp.innerHTML =  `${Math.round(response.current_weather.temperature)}°`;
  weatherTodaysHigh.innerHTML =  `${Math.round(response.daily_weather.data[0].temperatureHigh)}°`;
  weatherTodaysLow.innerHTML =  `${Math.round(response.daily_weather.data[0].temperatureLow)}°`;
...
function detailedWeather(response){
  ...
  detailedFeelsLike.innerHTML = `Feels Like: ${Math.round(response.current_weather.apparentTemperature)}°`;
...
function printHourForecast(hourly){
  $.each(hourly, function(index, data ) {
    ...
    z.innerHTML = `${Math.round(data.temperature)}°`;
  });
}
